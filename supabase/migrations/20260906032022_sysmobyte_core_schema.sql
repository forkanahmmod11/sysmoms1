...
- `profiles` — extends auth.users with display data (full_name, role, avatar_url, cover_url, bio, skills, department_id, status).
  - role: 'super_admin' | 'admin' | 'moderator' | 'employee' (and more added later)
...
3. Notes
- A trigger `handle_new_user` creates a profile row automatically when a new auth.users row is created, defaulting role to 'employee' (super_admin for ahmedforkan26@gmail.com).
*/
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  full_name text,
  email text,
  role text NOT NULL DEFAULT 'employee' CHECK (role IN ('super_admin','admin','moderator','employee')),
