...
import { PublicPostAdmin, UserProfileView } from '@/components/Community';
...
import {
  ...
  ShieldCheck, Sparkles, ToggleLeft, ToggleRight, UsersRound, X, Zap, KeyRound, Newspaper, UserRound, Eye,
} from 'lucide-react';

type SuperView = 'overview' | 'organizations' | 'members' | 'features' | 'projects' | 'subscriptions' | 'public_posts' | 'secrets' | 'profile';
...
export function SuperAdminPanel({ onLogout, onEnterWorkspace }: { onLogout: () => void; onEnterWorkspace: () => void }) {
  ...
  const [searchResults, setSearchResults] = useState<{ id: string; full_name: string | null; email: string | null; avatar_url: string | null }[]>([]);
  ...
  const navItems: { label: string; icon: typeof LayoutDashboard; view: SuperView; badge?: string }[] = [
    ...
    { label: 'Public Posts', icon: Newspaper, view: 'public_posts' },
    ...
    <main className="workspace super-admin">
      ...
      <section className="workspace-main">
        <header className="workspace-header">
          ...
          {searchOpen && <div className="search-popover master-search-popover"><Search size={17} /><input autoFocus value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users..." />
            {search.trim() && <MasterUserSearch query={search} onSelect={(id) => { setProfileUserId(id); setView('profile'); setSearchOpen(false); setSearch(''); }} />}
          </div>}
        </header>
        <div className="content-area">
          ...
          {view === 'public_posts' && <PublicPostAdmin />}
          {view === 'secrets' && <SecretsView />}
          {view === 'profile' && profileUserId && <UserProfileView userId={profileUserId} onBack={() => setView('members')} />}
        </div>
      </section>
    </main>
  );
...
function MasterUserSearch({ query, onSelect }: { query: string; onSelect: (id: string) => void }) {
  const [rows, setRows] = useState<{ id: string; full_name: string | null; email: string | null; avatar_url: string | null }[]>([]);
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      if (!supabase || !query.trim()) { setRows([]); return; }
      const q = query.trim();
      const { data } = await supabase.from('profiles').select('id, full_name, email, avatar_url').or(`full_name.ilike.%${q}%,email.ilike.%${q}%`).order('full_name').limit(8);
      setRows((data as typeof rows) || []);
    }, 180);
    return () => window.clearTimeout(timer);
  }, [query]);
  return <div className="master-search-results">
    {rows.map((r) => <button key={r.id} onClick={() => onSelect(r.id)}><span className="member-avatar">{initials(r.full_name || r.email || 'U')}</span><span><strong>{r.full_name || 'Unknown'}</strong><small>{r.email || ''}</small></span><Eye size={15} /></button>)}
    {!rows.length && <div className="empty-inline">No users found.</div>}
  </div>;
}

function SecretsView() {
  const [secrets, setSecrets] = useState<{ id: string; name: string; description: string | null; created_at: string; updated_at: string }[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [saving, setSaving] = useState(false);
  const load = useCallback(async () => {
    if (!supabase) return;
    const { data } = await supabase.rpc('list_platform_secrets');
    setSecrets((data as typeof secrets) || []);
  }, []);
  ...
  const save = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.rpc('upsert_platform_secret', {
      p_name: String(f.get('name') || '').trim(),
      p_value: String(f.get('value') || ''),
      p_description: String(f.get('description') || '').trim() || null,
    });
    setSaving(false);
    if (!error) { setShowForm(false); e.currentTarget.reset(); await load(); }
  };
  return <div className="section-view">
    <div className="page-heading"><div><div className="eyebrow dark"><span className="eyebrow-dot" /> Platform security</div><h1>Secrets</h1><p>Store API keys and other sensitive values in Supabase Vault. Values are never displayed back in the dashboard.</p></div><button className="primary-button small" onClick={() => setShowForm(!showForm)}><Plus size={17} /> Add secret</button></div>
    {showForm && <div className="panel form-panel"><form onSubmit={save}>
      <label className="form-field"><span>Secret name</span><input name="name" placeholder="STRIPE_SECRET_KEY" required /></label>
      <label className="form-field"><span>Secret value</span><input name="value" type="password" autoComplete="new-password" required /></label>
      <label className="form-field"><span>Description</span><input name="description" placeholder="Used by payment processing" /></label>
      <div className="form-actions"><button type="button" className="secondary-button" onClick={() => setShowForm(false)}>Cancel</button><button className="primary-button small" disabled={saving}>{saving ? 'Saving...' : 'Save secret'}</button></div>
    </form></div>}
    <div className="panel section-table"><div className="table-toolbar"><div className="table-tabs"><button className="selected">Vault secrets ({secrets.length})</button></div></div>
      {secrets.map((s) => <div className="table-row" key={s.id}><span className="project-bullet orange" /><div className="table-primary"><strong>{s.name}</strong><span>{s.description || 'No description'}</span></div><span className="role-badge">Encrypted</span><small>{new Date(s.updated_at).toLocaleString()}</small></div>)}
      {!secrets.length && <div className="empty-inline">No platform secrets have been added.</div>}
    </div>
  </div>;
}
    ...
    <div className="section-view">
      <div className="page-heading">
        <div>
          ...
          <p>Review payment proof and transaction data, then approve or reject subscription requests. Approval activates the user’s isolated workspace.</p>
        </div>
            ...
            {filtered.map((req) => (
              <div className="table-row subscription-row" key={req.id}>
                ...
                <div className="table-primary">
                  ...
                  <span>{req.plan_name} · {req.payment_method?.toUpperCase() || "Payment method not recorded"} · {req.organization_name}</span>
                </div>
              ...
              <div className="modal-panel" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="modal-body">
                  <div className="detail-grid">
                    ...
                    <div className="detail-item"><span>Payment method</span><strong>{selectedReq.payment_method ? selectedReq.payment_method.toUpperCase() : "Not recorded"}</strong></div>
                    <div className="detail-item"><span>Transaction ID / reference</span><strong>{selectedReq.payment_reference || "Not recorded"}</strong></div>
