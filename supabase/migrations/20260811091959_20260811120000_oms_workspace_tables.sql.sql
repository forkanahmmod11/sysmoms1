...
## Purpose
Creates the core tables that power the Office Management System (OMS) dashboard
for regular (non-super-admin) users. All tables are organization-scoped and
owner-aware, so each user sees only the data that belongs to their organization.

## New Tables

1. **tasks** — Individual work items assigned to users within an organization.
   - id, organization_id, user_id (assignee), title, description, status
     (todo / in_progress / review / done), priority (low / medium / high / urgent),
     project_id (optional link to projects), due_date, created_at, updated_at.

2. **events** — Calendar / schedule entries for the organization.
   - id, organization_id, user_id (creator), title, description, location,
     start_time, end_time, created_at.

3. **messages** — Direct messages between members of an organization.
   - id, organization_id, sender_id, receiver_id, content, read_at, created_at.

4. **transactions** — Financial transactions / expense tracking per organization.
   - id, organization_id, user_id (creator), title, amount, type
     (income / expense), category, status (pending / approved / rejected),
     created_at.

5. **notices** — Organization-wide announcements / notices.
   - id, organization_id, user_id (author), title, content, priority
     (normal / important / urgent), created_at.
...
- user_id columns default to auth.uid() so inserts from the client succeed
  even when the client omits the owner field.
*/
...
CREATE POLICY "select_org_events" ON events FOR SELECT
  TO authenticated USING (
    organization_id IS NULL OR
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = events.organization_id AND om.user_id = auth.uid())
  );
...
CREATE POLICY "select_org_transactions" ON transactions FOR SELECT
  TO authenticated USING (
    organization_id IS NULL OR
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = transactions.organization_id AND om.user_id = auth.uid())
  );
...
CREATE POLICY "select_org_notices" ON notices FOR SELECT
  TO authenticated USING (
    organization_id IS NULL OR
    EXISTS (SELECT 1 FROM organization_members om WHERE om.organization_id = notices.organization_id AND om.user_id = auth.uid())
  );
