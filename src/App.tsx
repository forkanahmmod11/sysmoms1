...
import { getWorkspaceAccess, type WorkspaceAccess } from '@/lib/workspaceAccess';
...
function App() {
  const { loading, session, profile, offlineAccess, signInWithEmail, signUpWithEmail, signInWithGoogle, signOut } = useAuth();
  ...
  const refreshWorkspaceAccess = async () => {
    if (hasSuperAdminAccess || !profile?.id) { setWorkspaceAccess(null); return; }
    setAccessLoading(true);
    const state = await getWorkspaceAccess(profile.id);
    setWorkspaceAccess(state);
    setAccessLoading(false);
  };

  useEffect(() => { if (screen === 'app' && profile?.id && !hasSuperAdminAccess) void refreshWorkspaceAccess(); }, [screen, profile?.id, hasSuperAdminAccess]);
  useEffect(() => {
    if (screen !== 'app' || !profile?.id || hasSuperAdminAccess || workspaceAccess?.fullAccess) return;
    const timer = window.setInterval(() => { void refreshWorkspaceAccess(); }, 5000);
    return () => window.clearInterval(timer);
  }, [screen, profile?.id, hasSuperAdminAccess, workspaceAccess?.fullAccess]);
  ...
  const handleAuth = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) { setAuthError(error); }
        ...
        setAuthInfo('Account created. If email verification is enabled, check your inbox and verify the Gmail/email address before signing in. You can also continue with Google anytime.');
  ...
  const handleGoogle = async () => {
    setAuthError(null); setAuthInfo(null); setAuthLoading(true);
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {screen === 'auth' && <AuthScreen mode={authMode} loading={authLoading} error={authError} info={authInfo} onSubmit={handleAuth} onGoogle={handleGoogle} onBack={() => setScreen('landing')} onSwitchMode={(m) => { setAuthMode(m); setAuthError(null); setAuthInfo(null); }} />}
        {screen === 'app' && (session || offlineAccess) && (appMode === 'platform' && hasSuperAdminAccess
          ...
          : hasSuperAdminAccess
            ? <Workspace onLogout={handleLogout} onEnterPlatform={enterPlatform} />
            : accessLoading || !workspaceAccess
              ? <SplashLoader />
              : !workspaceAccess.onboardingComplete
                ? <Onboarding onLogout={handleLogout} onComplete={refreshWorkspaceAccess} />
                : workspaceAccess.fullAccess
                  ? <Workspace onLogout={handleLogout} />
                  : <Workspace onLogout={handleLogout} limited />
        )}
