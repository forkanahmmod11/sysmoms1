...
export type View = 'overview' | 'subscription' | 'tasks' | 'projects' | 'team' | 'schedule' | 'messages' | 'community' | 'transactions' | 'settings' | 'profile';
...
const navItems: NavItem[] = [
  ...
  { label: 'Subscription', icon: CreditCard, view: 'subscription' },
  ...
  { label: 'Community', icon: Newspaper, view: 'community' },
];
  ...
  return (
    <main className="workspace">
      ...
      <aside className={`sidebar ${mobileMenu ? 'open' : ''}`}>
        ...
        <nav className="side-nav">
          <button disabled={limited} className={`${view === 'transactions' ? 'active' : ''} ${limited ? 'locked' : ''}`} onClick={() => { if (limited) return; setView('transactions'); setMobileMenu(false); }}>
            <CreditCard size={18} /><span>Transactions</span>
          </button>
          <button disabled={limited} className={`${view === 'settings' ? 'active' : ''} ${limited ? 'locked' : ''}`} onClick={() => { if (limited) return; setView('settings'); setMobileMenu(false); }}>
        ...
        <div className="content-area">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={view} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}>
              ...
              {view === 'profile' && profileUserId && <UserProfileView userId={profileUserId} onBack={() => setView('team')} />}
...
function Overview({ onView, profile }: { onView: (view: View) => void; profile: UserProfile | null }) {
  ...
  return (
    <div className="overview">
      ...
      <section className="panel dashboard-public-feed">
        <div className="panel-heading"><div><span className="panel-eyebrow">Community</span><h2>Public updates</h2></div><button className="panel-link" onClick={() => onView('community')}>View all <ArrowUpRight size={15} /></button></div>
        <PublicFeed limit={3} />
      </section>
