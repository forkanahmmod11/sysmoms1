...
import { getUserRequest } from '@/lib/subscription';
...
function App() {
  ...
  const [subStatus, setSubStatus] = useState<'none' | 'pending' | 'approved' | 'rejected' | 'checking'>('checking');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!req) setSubStatus('none');
      else if (req.status === 'approved') setSubStatus('approved');
  ...
  const handleAuth = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) { setAuthError(error); }
        ...
        setAuthInfo('Account created! Check your email or sign in with your new credentials.');
        setAuthMode('signin');
      }
    } catch {
        ...
        {screen === 'app' && (session || offlineAccess) && (profile?.role === 'super_admin'
          ...
          : subStatus === 'approved'
            ...
            : subStatus === 'pending' || subStatus === 'rejected'
              ? <PendingApproval onLogout={handleLogout} onApproved={() => setSubStatus('approved')} />
              : subStatus === 'none'
