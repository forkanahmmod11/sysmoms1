  ...
  return (
    <main className="workspace super-admin">
      <aside className={`sidebar ${mobileMenu ? 'open' : ''}`}>
        ...
        <div className="workspace-switcher super-badge">
...
function SuperOverview({ onNavigate }: { onNavigate: (view: SuperView) => void }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: orgData }, { data: memberData }, { data: featureData }, { data: projectData }] = await Promise.all([
      ...
      setStats({ orgs: memberData?.length || 0, members: memberData?.length || 0, features: featureData?.length || 0, projects: projectData?.length || 0 });
      setLoading(false);
    })();
  }, []);
