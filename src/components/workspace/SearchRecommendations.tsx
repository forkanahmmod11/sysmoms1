...
export function SearchRecommendations({ profile, onSelect }: { profile: UserProfile | null; onSelect: (target: SearchTarget) => void }) {
  ...
  useEffect(() => {
    const q = query.trim();
    if (!supabase || !profile?.organization?.id || q.length < 2) { setResults([]); return; }
    let alive = true;
    const run = async () => {
      setLoading(true);
      ...
      const [projects, tasks, members, messages] = await Promise.all([
        supabase.from('projects').select('id,name,description,created_at').eq('organization_id', orgId).or(`name.ilike.${pattern},description.ilike.${pattern}`).limit(8),
        supabase.from('tasks').select('id,title,description,created_at').eq('organization_id', orgId).or(`title.ilike.${pattern},description.ilike.${pattern}`).limit(8),
        supabase.from('organization_members').select('user_id,profiles(full_name,email)').eq('organization_id', orgId).limit(50),
        supabase.from('messages').select('id,content,created_at').eq('organization_id', orgId).ilike('content', pattern).limit(8),
      ]);
      if (!alive) return;
      const rows: Result[] = [];
