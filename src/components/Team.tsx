...
export function TeamView({ onOpenProfile }: { onOpenProfile?: (userId: string) => void }) {
      ...
      <div className="panel section-table">
        ...
        {loading ? (
          ...
          <>
            ...
            {filtered.map((m) => (
              <button className="table-row table-row-button" key={m.id} onClick={() => onOpenProfile?.(m.user_id)}>
